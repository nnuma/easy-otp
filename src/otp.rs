...
fn get_current_datetime_str(config: &Config) -> (String, i64) {
  ...
  let minutes_str = format!("{00}", interval_minutes * (minutes / interval_minutes));
  let datetime_str = now.format("%Y%m%d%H");
  let targ_datetime_str = now.format("%Y-%m-%dT%H:{}:00%:z").to_string().replace("{}", &format!("{:0>2}", minutes_str)).to_string();
  ...
  return (format!("{}{}", datetime_str.to_string(), minutes_str), remaining_seconds);
}
